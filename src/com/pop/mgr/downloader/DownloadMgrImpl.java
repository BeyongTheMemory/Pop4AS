...
public class DownloadMgrImpl implements Runnable, DownloadManager {
    ...
    public void run() {
        if (tag) {
            ...
            param.put("lat",olatitude);
            param.put("lon",olongitude);
            try {
                String result = HttpUtils.doPost(UrlUtil.getPop(),param);
                ...
                if (popResponse.getResult() == ClientCode.SUCCESS) {
                    ...
                    if (!CollectionUtil.isEmpty(popDtos)) {
                        ...
                        for (PopDto popDto : popDtos) {
                            ...
                            imge.setType(popDto.getType() + "");
                            markers.add(imge);
                        }
                    }else{
                        Toast.makeText(x.app(), "您的附近暂没有泡泡哟,来自己发一个吧", Toast.LENGTH_LONG).show();
                    }
                }else {
